...
Plans generated by users or LLMs often lack sufficient detail for implementation. The coordinator assesses each task before developer assignment and spawns business analyst agents to expand underspecified tasks.
...
**Critical Rule:** The coordinator must NOT dispatch developers for tasks classified as `NEEDS_EXPANSION` or `NEEDS_CLARIFICATION`. Only `IMPLEMENTABLE` tasks enter the `available_tasks` queue.
